...
float getPosition(){
  ...
  float sum = 0;
  for(int i = 0; i < 6; i++){
    float value = (float)getValue(i)*1000; // (SENSOR_MAX - SENSOR_MIN) + 1.0;
    weighted_sum = weighted_sum + value  * (i);
    sum = sum + value;
  }
  // Serial.println();
...
void main() {
    ...
    while(1) {
        ...
        logOut.printf("%d: %.10f \t", "POSICAO", getPosition());
        logOut.printf("\n");
        wait(.25);
//        for (int i = 0; i < 6; i++)
//            logOut.printf("%d: %.10f \t", i, getValue(i));
//        logOut.printf("\n");
//        wait(.25);
        //
//        outPID = PID_update(position,1);
//        logOut.printf("%d: %.10f \t", "PID", outPID);
//        logOut.printf("\n");
